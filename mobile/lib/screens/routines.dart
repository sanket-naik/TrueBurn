...
class _RoutinesScreenState extends State<RoutinesScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    ({int done, int due}) tally(bool isYest) {
      ...
      for (final r in active) {
        ...
        // exists. Both are written by the same rollover, so they cannot disagree.
            ...
            ? (r.history[yesterday] ?? r.doneYesterday.length)
            : r.done.length;
        due += r.times.length;
      }
    ...
    final t = tally(false);
    final missed =
        active.fold(0, (a, r) => a + missedTimes(r, dow, nowMin, s.paused, today).length);
    ...
    return ListView(
      ...
      children: [
        Row(children: [
          Expanded(
              child: Text('Routines', style: screenTitle(c))),
            ...
            onLongPress: !kDebugMode || active.isEmpty
                ...
                : () {
                    Notifications.fireTestIn(
                        const Duration(seconds: 10), active.first, active.first.times.first);
                    ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(content: Text('Test reminder in 10s')));
                  },
        ...
        Row(children: [
          Expanded(
            child: PrimaryButton('New routine', onTap: () {
              showRoutineSheet(context, s, s.newRoutine(RoutineType.water), true)
                  .then((_) => setState(() {}));
            }),
          ),
          if (!s.paused) ...[
    ...
    showAppSheet(context, (ctx) {
      return Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const SizedBox(height: 8),
          Text('Nothing fires until then. Day counts keep running, so a pause costs no progress.',
              style: sans(c)),
          ...
          for (final o in [
            ...
            Pill(
              '${o.$2} · until ${o.$1 == -1 ? 'tomorrow morning' : o.$1 == -2 ? 'midnight' : ampm(clockOf(nowMin + o.$1))}',
              onTap: () {
                setState(() => s.pauseFor(o.$1 == -1 ? -1 : o.$1 == -2 ? 1440 : nowMin + o.$1));
                Navigator.pop(ctx);
              },
            ),
            const SizedBox(height: 8),
          ...
          Row(children: [
            ...
            Expanded(
              child: Text.rich(TextSpan(children: [
                ...
                TextSpan(
                    // Denominator counts only the slots that were ever going to fire
                    // today, so a routine added at 11 pm does not read "0 of 4".
                    text: ' of $tracked ${tracked == r.times.length ? 'done today' : 'due since you added it'}',
                    style: sans(c, size: 12, color: miss.isNotEmpty ? c.warn : c.ink2)),
              ])),
            ),
          ])
        else
